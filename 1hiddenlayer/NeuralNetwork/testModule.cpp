...
void forwardProcedureTesting() {
	...
	for (int i = 1; i <= hiddenNeuronNumber; ++i) {
		for (int j = 1; j <= outputNeuronNumber; ++j) {
			outputLayerIN[j] += hiddenLayerOUT[i] * weightstHiddenOutput[i][j];
		}
	}
...
void summary(int nCorrect)
{
	...
	report << "Epochs: " << epochs << "Learning rate: " << learning_rate << "Epsilon: " << epsilon << "Hidden Neuron count: " << hiddenNeuronNumber << endl;
	report << "Samples learned: " << trainingDataSamplesCounter << endl; 
}
