...
void initializeTrainModule() {
	...
	for (int i = 1; i <= hiddenNeuronNumber; ++i) {
		...
		delta2[i] = new double[outputNeuronNumber + 1];
	}
...
void updateWeights()
{
	for (int i = 1; i <= hiddenNeuronNumber; ++i) {
		for (int j = 1; j <= outputNeuronNumber; ++j) {
			delta2[i][j] = (learning_rate * deltaOutputSum[j] * hiddenLayerOUT[i]) + (momentum * delta2[i][j]);
			weightstHiddenOutput[i][j] += delta2[i][j];
		}
	}
